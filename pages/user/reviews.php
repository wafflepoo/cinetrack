...
.review-card { transition: .2s; border: 1px solid transparent; }
...
<div class="review-item film-review review-card glass p-6 rounded-xl relative group">
    ...
    <!-- DELETE BUTTON (FIXED) -->
    <button onclick="deleteReview(<?= (int)$review['id_critique'] ?>, 'film')" 
    ...
    <div class="flex gap-6">
        ...
        <div class="flex-grow">
            <div class="flex justify-between mb-2">
                ...
                <div class="rating-circle <?= getRatingColor($review['note']) ?>">
                    <?= number_format($review['note'], 1) ?>
                </div>
            </div>
            ...
            <p class="text-gray-500 text-sm">
                Posté le <?= date('d/m/Y à H:i', strtotime($review['date'])) ?>
            </p>
        </div>
    </div>
</div>
...
<div class="review-item serie-review review-card glass p-6 rounded-xl relative group">
    ...
    <button onclick="deleteReview(<?= (int)$review['id_critique'] ?>, 'serie')" 
        class="absolute top-4 right-4 bg-red-600 text-white w-8 h-8 rounded-full hidden group-hover:flex items-center justify-center">
        ✕
    </button>
...
function deleteReview(id, type) {
    ...
    data.append("id_review", id);
    data.append("type", type);

    fetch("delete-review.php", { method: "POST", body: data })
    .then(r => r.json())
    .then(res => {
...
document.querySelectorAll(".tab-btn").forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        items.forEach(card => {
            ...
            else if (filter === "film") card.style.display =
                card.classList.contains("film-review") ? "block" : "none";
            else if (filter === "serie") card.style.display =
                card.classList.contains("serie-review") ? "block" : "none";
        });
    });
});
</script>
