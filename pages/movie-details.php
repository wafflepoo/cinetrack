...
<div class="flex flex-wrap gap-4 mb-6 justify-center lg:justify-start">
    ...
    <button onclick="addToCustomList()" class="glass px-6 py-3 rounded-lg font-semibold border border-orange-500/50 hover:bg-orange-500/20 transition">
...
function addToCustomList() {
    ...
    <?php if (isLoggedIn()): ?>
        ...
        loadUserListsForDetails(movieData);
    <?php else: ?>
...
// Fonction pour charger les listes de l'utilisateur
function loadUserListsForDetails(movieData) {
    fetch('lists/get-user-lists.php') 
        .then(response => response.json())
        .then(lists => {
            showListSelectionModalDetails(lists, movieData);
        })
        .catch(error => {
...
// Fonction pour afficher la modal de sélection de liste (adaptée pour movie-details)
function showListSelectionModalDetails(lists, movieData) {
    const modalId = 'listModal-details-' + movieData.movie_id;
        ...
        <div id="${modalId}" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100]">
            <div class="bg-gray-900 border border-orange-500/30 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
                ...
                <p class="text-gray-300 mb-6">
                    Choisissez une liste pour "<strong class="text-orange-400">${movieData.movie_title}</strong>"
                </p>
                
                <div id="listsContainer-details-${movieData.movie_id}" class="space-y-3 max-h-60 overflow-y-auto mb-6 pr-2">
                    ${lists.length === 0 ? 
                ...
                <div class="flex gap-3">
                    <button onclick="createNewListFromDetails(${movieData.movie_id}, '${movieData.movie_title.replace(/'/g, "\\'")}', '${movieData.movie_poster.replace(/'/g, "\\'")}', '${movieData.release_date.replace(/'/g, "\\'")}', '${modalId}')"
    ...
    if (lists.length > 0) {
        const container = document.getElementById(`listsContainer-details-${movieData.movie_id}`);
        lists.forEach(list => {
            ...
            listItem.innerHTML = `
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button onclick="addToSpecificListFromDetails(${list.id_liste}, ${movieData.movie_id}, '${movieData.movie_title.replace(/'/g, "\\'")}', '${movieData.movie_poster.replace(/'/g, "\\'")}', '${movieData.release_date.replace(/'/g, "\\'")}', '${modalId}')"
...
// Fonction pour ajouter à une liste spécifique
function addToSpecificListFromDetails(listId, movieId, movieTitle, moviePoster, releaseDate, modalId) {
    // D'abord ajouter le film à la base de données (si pas déjà)
    const movieData = {
        movie_id: movieId,
...
// Fonction pour créer une nouvelle liste depuis movie-details
function createNewListFromDetails(movieId, movieTitle, moviePoster, releaseDate, modalId) {
...
// Fonction pour fermer la modal (ajoutez-la si elle n'existe pas)
